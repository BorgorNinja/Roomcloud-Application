...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_FILES['profile_picture'])) {
    ...
    if (move_uploaded_file($_FILES["profile_picture"]["tmp_name"], $newFileName)) {
        ...
        $updateQuery = "UPDATE logindata SET profile_picture=? WHERE LOWER(email)=LOWER(?)";
        $updateStmt = $conn->prepare($updateQuery);
        if ($updateStmt === false) {
            die('Prepare failed: ' . htmlspecialchars($conn->error));
        }
        $updateStmt->bind_param('ss', $newFileName, $username);
        $updateStmt->execute();
        $updateStmt->close();
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['remove_picture'])) {
    $defaultPicture = 'https://via.placeholder.com/100';
    $updateQuery = "UPDATE logindata SET profile_picture=? WHERE LOWER(email)=LOWER(?)";
    $updateStmt = $conn->prepare($updateQuery);
    if ($updateStmt === false) {
        die('Prepare failed: ' . htmlspecialchars($conn->error));
    }
    $updateStmt->bind_param('ss', $defaultPicture, $username);
    $updateStmt->execute();
    $updateStmt->close();
...
if ($student) {
    ...
    $profilePicture = htmlspecialchars($student['profile_picture']) ?: 'https://via.placeholder.com/100';
} else {
                    ...
                    <div class="card mb-4">
                        <div class="card-body">
                            <div class="profile-info">
                                <img src="<?php echo $profilePicture; ?>" class="rounded-circle" height="100px" width="100px" alt="Profile Picture">
                                <div class="moved-slightly-right">
                                    ...
                                    <form action="studentprofile.php" method="post" enctype="multipart/form-data"class="form-inline">
                                        <input type="file" name="profile_picture" class="form-control-file">
                                        <button type="submit" class="btn btn-secondary ml-2">Upload</button>
                                        <button type="submit" name="remove_picture" class="btn btn-danger ml-2">Remove</button>
                                    </form>
                                </div>
